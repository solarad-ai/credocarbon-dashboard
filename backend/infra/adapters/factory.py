...
Usage:
    from backend.infra.adapters.factory import AdapterFactory
...
def _load_local_adapters():
    ...
    from backend.infra.local.adapters import (
...
def _load_gcp_adapters():
    ...
    try:
        ...
        try:
            from backend.infra.gcs.adapters import GCSFileStorageAdapter
            register_provider("gcp", ProviderConfig(
                file_storage=GCSFileStorageAdapter,
            ))
        except ImportError:
...
def _load_aws_adapters():
    ...
    try:
        from backend.infra.aws.adapters import (
...
def _load_azure_adapters():
    ...
    try:
        from backend.infra.azure.adapters import (
